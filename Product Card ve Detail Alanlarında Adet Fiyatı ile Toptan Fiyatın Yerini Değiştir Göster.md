...
$adet_fiyat = $koli_fiyat / $koli_adedi;
    ...
    <div class="price-wrapper-custom">
        ...
        <!-- ADET FİYATI (ANA FİYAT) -->
        <?php if ( is_user_logged_in() ) : ?>
            <div class="unit-main-price">
              Adet Fiyatı:  <?php echo wc_price($adet_fiyat); ?>
            </div>
        <?php else : ?>
        ...
        <!-- KOLİ FİYATI (ALTTA KÜÇÜK) -->
			<div class="box-sub-price" style="font-size:12px;color:#777;">
    			<?php if ( is_user_logged_in() ) : ?>
        			Koli Fiyatı (<?php echo $koli_adedi; ?> adet):
    			<?php endif; ?>// temadan ayarlandığı için fiyatı devamlı göster dedim
    			<?php echo $price; ?>
			</div>
